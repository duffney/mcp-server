...
//TODO: GetPatchedTags
//NOTE: note sure if this is needed or if the llm can pull tags from copa output?
// func (c *CLI) GetPatchedTags() (tags []string, err error) {
// 	customTagProvided := c.tag != ""
// 	platformProvided := len(c.platforms) == 0
// 	multiplatform, err := IsMultiPlatform(c.context, c.image)
// 	if err != nil {
// 		return []string{}, fmt.Errorf("failed to parse image reference %s: %w", c.image, err)
// 	}
//
// 	if customTagProvided && !multiplatform && !platformProvided {
// 		return []string{c.tag}, nil
// 	}
//
// 	if !customTagProvided && !platformProvided && !multiplatform {
// 		return []string{c.image + patchedSuffix}, nil
// 	}
//
// 	if multiplatform && !platformProvided {
// 		for _, p := range GetAllSupportedPlatforms() {
// 			arch := PlatformToArch(p)
// 			tags = append(tags, fmt.Sprintf("%s-%s", c.image, arch))
// 		}
// 	}
//
...
// 		}
//
// 		tags = append(tags, tag+patchedSuffix)
// 	}
//
// 	if len(c.platforms) > 0 {
// 		platforms := FilterSupportedPlatforms(c.platforms)
// 		for _, p := range platforms {
// 			arch := PlatformToArch(p)
// 			tags = append(tags, fmt.Sprintf("%s:%s-%s", repository, tag, arch))
// 		}
// 	}
//
// 	return tags, nil
